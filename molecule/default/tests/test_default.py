...
def AnsibleVars(Ansible):
    return Ansible("include_vars",
                   os.path.join("../../defaults/main.yml"))["ansible_facts"]
...
def test_keystone_projects(host, AnsibleVars, capsys):
    ...
    for project in projects:
        ...
        cmd = host.check_output(openstack_wrapper(AnsibleVars,
                                subcmd=subcmd, auth_url=address))
        project_results.append(cmd)

    for result in project_results:
        json_result = json.loads(result)
        assert json_result['name'] in projects
        assert json_result['enabled']
